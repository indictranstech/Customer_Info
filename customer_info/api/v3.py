...
# 	customer ='Anuradha K'
	...
	#print "and open agreemets :",customer_agreements
	...
	#print "We have payments",payments
					...
					if row.check_box_of_submit == 0 and row.payment_id == payment:
						...
						if (getdate(row.due_date) < firstDay_of_month or getdate(row.due_date) < now_date):
							...
							if float(flagged_receivables) >= total_charges:
								...
								campaign_discount = frappe.get_doc("Customer Agreement",customer_agreement.name).campaign_discount
								discounted_payments_left = frappe.get_doc("Customer Agreement",customer_agreement.name).discounted_payments_left
